...
What consumers do with the events is up to them. The same event can drive a Slack message, a PagerDuty page, a CI run, or an automated remediation pipeline. A separate proposal builds a guided improvement workflow on top of these events, but this RFC stands on its own.
...
3. **The quality signal.** Not every request hits the broken tool, so the agent limps along answering worse. As automatic evaluations score the sampled traffic, the correctness rolling average crosses the threshold and the quality webhook fires. The team's endpoint routes it to Slack:
