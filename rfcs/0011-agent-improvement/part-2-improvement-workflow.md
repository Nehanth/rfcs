...
Part 1 gives MLflow the ability to send a signal when an agent degrades. This RFC proposes what acts on that signal: an **Improve** tab in the experiment UI where the user connects the agent's repository, reads a diagnosis of what went wrong and where in the code it points, and approves a fix that arrives as a pull request.
...
# runs when Part 1's events fire; list the open ones and apply one
...
1. The threshold event fires on the rolling average and triggers the workflow. The diagnosis points at the prompt rather than the code.
...
None of this state exists today. The proposal adds four new pieces of stored state; everything else the workflow produces lands in storage MLflow already has — improved prompts in the prompt registry, regression traces in evaluation datasets, credentials in the existing secrets store, job records in the job framework's own tables. The threshold rules and their cooldown state belong to Part 1 and are stored there.

1. **The Improve configuration**, one per experiment: the source location, the backend chosen for each of the three interfaces, credential references into the existing secrets store for the acquisition and submission backends (never the tokens themselves), the trigger settings — which Part 1 events start an analysis for this experiment — and the optional detection schedule.
...
- **New detection capabilities.** This RFC consumes issue detection and the Part 1 events; it does not extend them.
...
flowchart TD
    ...
    E1 -.-> WH[Part 1 webhooks:<br/>Slack, PagerDuty, CI]
    E2 -.-> WH
...
Dotted lines are Part 1's webhook consumers, which work with or without this workflow. Everything from the trigger down is this RFC.
